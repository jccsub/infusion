...
export class InfusionContextMssqlWriter implements InfusionContextWriter {
  ...
  public async write(context: InfusionContext) {
    ...
    if (this.isIrrelevantUrl(context)) {
      return;
    }

    if (this.isIrrelevantResponseTypes(context)) {
    ...
    try {
      ...
      .input('modifications', sql.VarChar(4000), JSON.stringify(context.config.modifications))
  ...
  private isIrrelevantUrl(context : InfusionContext) : boolean {
    return (context.config.irrelevantUrlSubstrings.some((s) => {
      return context.request.fullUrl.toLowerCase().indexOf(s) >= 0;
    }));
  }

  private isIrrelevantResponseTypes(context : InfusionContext) : boolean {
    if (!context.response.headers) {
      return false;
    }
    context.config.irrelevantResponseContentTypes.forEach((pattern) => {
      let contentType = context.response.headers['content-type'];
      if (contentType) {
